...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			\toprule[2pt]
			类 Class & 对象 Object & 含义 Meaning & 实例 Instance & 来源 Origin \\ \midrule[1.2pt]
			实点 dot & 1 个上实点 $\dot{~}$ & 将对象提升为（不同于 $\hat{~},\check{~},\tilde{~}$ 的）变量 & $\dot{1}, \dot{2}$ & \blabel{hook:Dot} \\ 
...
%			& 3 个上实点 $\dot{\ddot{~}}$ & 该转换张量对应的“结果张量”提升 3 阶 & $\dot{\ddot{\chi}}$ & \blabel{hook:dddot} \\ \midrule
			...
			& 1 个上尖帽 $\hat{~}$ & 将对象提升为（不同于 $\check{~},\dot{~},\tilde{~}$ 的）变量 or 归一化矢量 & $\hat{1}, \hat{e}$ & \blabel{hook:hat} \\ 
			& 1 个上波浪 $\tilde{~}$ & 将对象提升为（不同于 $\hat{~},\check{~},\dot{~}$ 的）变量 & $\tilde{1}$ & \blabel{hook:tilde} \\ 
			& 1 个下尖帽 $\check{~}$ & 将对象提升为（不同于 $\hat{~},\dot{~},\tilde{~}$ 的）变量 & $\check{1}, \check{\symup{\jmath}}, \check{\jmath}$ & \blabel{hook:check} \\ \midrule
			...
			中线 mid & 1 条中中线 \textcolor{gray}{--} & 自变量中含横向 2 维实空间频率 $\mathcolor{gray}{\bar{k}_{\symup{\rho}}}$ 的因变量 & $\Xint{\mathcolor{gray}{-}}{26}{D}^{\;\!\mathcolor{gray}{\omega}}_{\;\! \symup{\iota}\mathcolor{gray}{z}}, \Xint{\mathcolor{gray}{-}}{25}{E}^{\;\!\mathcolor{gray}{\omega}}_{\;\! \symup{\iota}\mathcolor{gray}{z}}, \Xint{{}^{}_{\mathcolor{gray}{-}}}{23}{\chi}$ & \blabel{hook:Xint} \\ \midrule
