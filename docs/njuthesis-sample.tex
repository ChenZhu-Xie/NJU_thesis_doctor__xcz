...
} % https://github.com/nju-lug/NJUThesis/discussions/103#discussioncomment-2535481
...
%		aftername = {},
%}}
%\renewcommand\thesubsubsection{（\arabic{subsubsection}）} %群友 子非
...
% https://tex.stackexchange.com/a/583976/309711
...
\ExplSyntaxOff
\getenv[\Desktop]{Desktop}
\newcommand{\mypath}{\Desktop/tex/latex/local/template/Barlow/}
 % 机魔 PC 的 \Desktop 含中文 folder ``桌面''，
 % 解决方案：不能更改注册表中的 中文 系统变量中的 %Desktop% 键-值中的 值 中的 ``...\桌面\...'' 为 ``...\Desktop\...''，
 % 而需要：在保持这个的前提下，去 win11 时间和语言 → 语言和区域 → 管理语言设置 → 更改系统区域设置 → 勾选 “Beta 版： 使用 Unicode UTF-8 提供全球语言支持(U)”
 % 当然，还需要："双引号"，和 “\renewcommand 自动展开命令后，再总地传入参数” 的这 2 个操作配合...
...
% 设置西文字体
\setmainfont{LXGW WenKai GB}     % 主体字体
% \setsansfont{Noto Sans SC}       % 无衬线字体
% \setmonofont{Ubuntu Mono}        % 等宽字体

% 设置中文字体
\setCJKmainfont{LXGW WenKai GB}       % 中文正文
% \setCJKsansfont{Noto Sans SC}         % 中文无衬线
...
%			& 3 个上实点 $\dot{\ddot{~}}$ & 该转换张量对应的“结果张量”提升 3 阶 & $\dot{\ddot{\chi}}$ & \blabel{hook:dddot} \\ \midrule
	...
	上短 other & 1 个上波浪 $\tilde{~}$ & “泛函/变分式”对变量的微扰 or 也作用于 $\mathcolor{gray}{t}$ 域的算符 & $\tilde{\varepsilon}^{\;\! \mathcolor{gray}{t} \hat{1}}_{\;\! \symup{\iota} \mathcolor{gray}{z} \textcolor{Maroon}{(1)}}, \tilde{\zeta}^{\;\! \mathcolor{gray}{t} \hat{1} \mathcolor{gray}{\check{1} \check{2}}}_{\;\! \symup{\iota} \mathcolor{gray}{z} \textcolor{Maroon}{(1)}}$ & \blabel{hook:tilde} \\ 
