...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			...
			粗细 width & 细体 thin\hphantom{k} & 每一个常量与变量 & $\bar{\bar{\chi}},\bar{\bar{\varepsilon}}$ & \blabel{Maroon} \\
			& \textbf{粗体 thick} & 未定义；保留的关键模式 & $\bar{\bar{\bar{\chi}}}$ & \blabel{Maroon} \\ \midrule
			下线 down & 0 根下短线 \hphantom{$\underline{~}$} & $\in$ 实验（室）坐标系 = $\mathcal{Z}$ 系（下的物理量） & $\bar{g}$ & \blabel{Maroon} \\
			& 1 根下短线 $\underline{~}$ & $\in$ 晶体（学）坐标系 = $\mathcal{C}$ 系（下的物理量） & $\bar{\underline{g}}$ & \blabel{Maroon} \\ \midrule
			底标 other & 1 根下短波浪线 $\uwav{~}$ & $\in$ 另一个（地点、运动状态的）参考系 & $\textcolor{gray}{\uwav{t}}, \textcolor{gray}{\bar{\uwav{r}}}$ & \blabel{uwav} \\ 
			& 1 个下空圈 $\uo{~}$ & $\in$ （所考虑的）事件/对象 本身的参考系 & $\textcolor{gray}{\uo{t}}$ & \blabel{uo} \\ \midrule
			长线 long & 1 根上长线 $\overline{~~}$ & 广义列向量，其中每个元素可能都是列向量 & $\underline{\bar{g}}$ & \blabel{Maroon} \\
			& 1 根下长线 $\underline{~~}$ & 未定义；保留的关键修饰符 &  & \blabel{Maroon} \\ 
			\bottomrule[2pt]
		\end{tabular}
	}
\end{table}
...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			...
			上线 up & 2 条上短线 $\bar{\bar{~}}$ & \hphantom{列向量 =} 张量：2 阶张量 & $\bar{J}_{\;\!\textcolor{Maroon}{\text{f}}}$ 中的 $\bar{J}$ & \blabel{Maroon} \\
			& 3 条上短线 $\bar{\bar{\bar{~}}}$ & \hphantom{列向量 =} 张量：3 阶张量 & $\bar{\bar{\bar{\chi}}}$ & \blabel{Maroon} \\ \midrule
			长线 long & 1 条上长线 $\overline{~~}$ & 广义列向量，其中每个元素可能都是列向量 & $\underline{\bar{g}}$ & \blabel{Maroon} \\
			...
			& 1 条下短线 $\underline{~}$ & $\in$ 晶体（学）坐标系 = $\mathcal{C}$ 系（下的物理量） & $\bar{\underline{g}}$ & \blabel{Maroon} \\ 
			\bottomrule[2pt]
		\end{tabular}
	}
\end{table}
