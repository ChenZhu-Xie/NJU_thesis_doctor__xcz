...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			...
			字体 font & 直体 upright & 离散变量（如$\symup{i}$分量）、常量、文字（说明） & $\upmu_0$ 中的 $\upmu$ 和 $0$ & \blabel{upright} \\
			& \textit{斜体 oblique} & 连续变量（自变量或因变量）、场量、函数 & $\bar{E}$ 中的 $E$ & \blabel{oblique} \\ \midrule
...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			...
			& 1 个上尖帽 $\hat{~}$ & 将对象提升为变量 or 归一化矢量 & $\mathcolor{gray}{\hat{1}},\mathcolor{gray}{\hat{e}}$ & \blabel{hat} \\ \midrule
