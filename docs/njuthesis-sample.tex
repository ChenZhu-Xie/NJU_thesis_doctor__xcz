...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			...
			底标 other & 1 条下短波浪线 $\uwav{~}$ & $\in$ 另一个（地点、运动状态的）参考系 & $\textcolor{gray}{\uwav{t}}, \textcolor{gray}{\bar{\uwav{r}}}$ & \blabel{uwav} \\ 
			& 1 个下空圈 $\uo{~}$ & $\in$ （所考虑的）事件/对象 本身的参考系 & $\textcolor{gray}{\uo{t}}$ & \blabel{uo} \\
			\bottomrule[2pt]
		\end{tabular}
	}
\end{table}
