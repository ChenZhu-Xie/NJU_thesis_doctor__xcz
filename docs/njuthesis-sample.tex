...
% 可以在这里插入补充材料
\appchapter{正文中的符号/排版/样式约定}

\vspace*{-1.0em}

\appsection{各数学字体、上下标、装饰符、颜色的含义}

\begin{table}[h!]
\bicaption{The meaning of mathematical fonts in distinguishing variables.}{\label{tab:font} 数学字体 --- 用于区分变量与常量、不同类变量，的含义}
%\bicaption
...
\begin{tabular}{c|c|c|c|c}
	...
	& 无衬线体 $\mathsf{mathsf}$ & 另一个（相关）物理量，或（转置）算符 & $\overline{\Xint{\mathcolor{gray}{-}}{25}{\bar{\mathsfit{v}}}^{\;\! \mathcolor{gray}{\omega} \textcolor{OliveGreen}{\imath}}_{\textcolor{OliveGreen}{(1)} \textcolor{RoyalPurple}{\jmath}}}^\mathsf{T}$ 中的 $\mathsfit{v}$ 和 $\mathsf{T}$ & \blabel{hook:mathsf} \\ \midrule
%	& 哥特字体 $\mathfrak{mathfrak}$ & 另一个（相关）物理量，如非相位部分 $\to$ 偏振态 & $\Xint{{}^{}_{\mathcolor{gray}{-}}}{08}{\bar{\mathsfit{g}}}^{\;\!\mathcolor{gray}{\omega} \textcolor{Maroon}{\jmath}}$ & \blabel{hook:mathfrak} \\ \midrule
	...
	& \textbf{粗体 bold} & 另一个（相关的）物理量，以缓解符号耗尽 & & \blabel{hook:bold} \\
	\bottomrule[2pt]
\end{tabular}
}
\end{table}
...
\begin{table}[h!]
\bicaption{The implications of lines as decorations applied to variables.}{\label{tab:line} 上/中/下 \& 长/短 划线 --- 作为变量的装饰，的含义}
\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
\begin{tabular}{c|c|c|c|c}
	\toprule[2pt]
	类 Class & 对象 Object & 含义 Meaning & 实例 Instance & 来源 Origin \\ \midrule[1.2pt]
...
%			& 3 个上实点 $\dot{\ddot{~}}$ & 该转换张量对应的“结果张量”提升 3 阶 & $\dot{\ddot{\chi}}$ & \blabel{hook:dddot} \\ \midrule
	...
	长线 long & 1 条上长线 $\overline{~~}$ & 广义列向量，其中/下每个元素可能都是列向量 & $\overline{\Xint{\begin{smallmatrix} ~ \\ {}^{}_{\mathcolor{gray}{-}} \\ ~ \end{smallmatrix}}{20}{\bar{\lambda}}^{\;\!\mathcolor{gray}{\omega}}_{\textcolor{Maroon}{\jmath}}}^\mathsf{T}$ & \blabel{hook:overline} \\
	...
	下线 down & 1 条下短线 $\underline{~}$ & $\in$ 晶体（学）坐标系 = $\mathcal{C}$ 系（下的物理量） & $\bar{\underline{g}}$ & \blabel{hook:crystal} \\ \midrule 
...
\begin{table}[h!]
\bicaption{The roles of colors for clarifying variable types and text.}{\label{tab:color} 颜色 --- 用于区分变量与文字、变量的类型，的含义}
\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
\begin{tabular}{c|c|c|c|c}
	\toprule[2pt]
	类 Class & 对象 Object & 含义 Meaning & 实例 Instance & 来源 Origin \\ \midrule[1.2pt]
	...
	& \textcolor{OliveGreen}{橄榄绿 OliveGreen} & （另一种）起补充说明作用的文字 or 变量（\textcolor{OliveGreen}{进阶·晶体光学}） & $\Xint{\begin{smallmatrix} ~ \\ {}^{}_{\mathcolor{gray}{-}} \\ ~ \end{smallmatrix}}{15}{\bar{v}}^{\;\!\mathcolor{gray}{\omega}}_{\textcolor{OliveGreen}{\jmath}}$ 中的 $\textcolor{OliveGreen}{\jmath}$ & \blabel{hook:OliveGreen} \\
	& \textcolor{RoyalPurple}{皇家紫 RoyalPurple} & （另一种）起补充说明作用的文字 or 变量（\textcolor{RoyalPurple}{进阶·数学名词}） & $\Xint{\mathcolor{gray}{-}}{25}{\bar{\mathsfit{v}}}^{\;\! \mathcolor{gray}{\omega} \textcolor{OliveGreen}{\imath}}_{\textcolor{OliveGreen}{(1)} \textcolor{RoyalPurple}{\jmath}}$ 中的 $\textcolor{RoyalPurple}{\jmath}$ & \blabel{hook:RoyalPurple} \\ 
	& \textcolor{MidnightBlue}{深夜蓝 MidnightBlue} & （另一种）起补充说明作用的直体文字（\textcolor{MidnightBlue}{进阶·物理术语}） & $\Xint{\mathcolor{gray}{-}}{25}{\bar{\mathsfit{v}}}^{\;\! \mathcolor{gray}{\omega} \textcolor{OliveGreen}{\imath}}_{\textcolor{OliveGreen}{(1)} \textcolor{RoyalPurple}{\jmath}}$ 中的 $\textcolor{RoyalPurple}{\jmath}$ & \blabel{hook:MidnightBlue} \\ \bottomrule[2pt]
\end{tabular}
}
\end{table}
