...
\begin{table}[h!]
	...
	\resizebox{1.0\linewidth}{!}{  % 宽度不超文本宽度
		\begin{tabular}{c|c|c|c|c}
			\toprule[2pt]
			类 Class & 对象 Object & 含义 Meaning & 实例 Instance & 来源 Origin \\ \midrule[1.2pt]
			实点 dot & 1 个上实点 $\dot{~}$ & 将对象提升为（不同于 $\hat{~},\check{~}$ 的）变量 & $\dot{1}, \dot{2}$ & \blabel{hook:Dot} \\ 
			...
			& 1 个上尖帽 $\hat{~}$ & 将对象提升为（不同于 $\check{~},\dot{~}$ 的）变量 or 归一化矢量 & $\hat{1}, \hat{e}$ & \blabel{hook:hat} \\ 
			& 1 个下尖帽 $\check{~}$ & 将对象提升为（不同于 $\hat{~},\dot{~}$ 的）变量 & $\check{1}, \check{\symup{\jmath}}, \check{\jmath}$ & \blabel{hook:check} \\ \midrule
